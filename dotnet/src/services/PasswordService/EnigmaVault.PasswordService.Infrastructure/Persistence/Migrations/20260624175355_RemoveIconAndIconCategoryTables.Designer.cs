// <auto-generated />
using System;
using EnigmaVault.PasswordService.Infrastructure.Persistence.Contexts;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace EnigmaVault.PasswordService.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(EnigmaContext))]
    [Migration("20260624175355_RemoveIconAndIconCategoryTables")]
    partial class RemoveIconAndIconCategoryTables
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("EnigmaVault.PasswordService.Domain.Models.Folder", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("Id");

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("Color");

                    b.Property<string>("FolderName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("FolderName")
                        .UseCollation("case_insensitive");

                    b.Property<Guid?>("ParentFolderId")
                        .HasColumnType("uuid")
                        .HasColumnName("ParentFolderId");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("UserId");

                    b.HasKey("Id");

                    b.HasIndex("ParentFolderId");

                    b.ToTable("Folders", (string)null);
                });

            modelBuilder.Entity("EnigmaVault.PasswordService.Domain.Models.Tag", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("Color");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("IconName")
                        .UseCollation("case_insensitive");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("UserId");

                    b.HasKey("Id");

                    b.ToTable("Tags", (string)null);
                });

            modelBuilder.Entity("EnigmaVault.PasswordService.Domain.Models.VaultItem", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("Id");

                    b.Property<DateTime>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("DateAdded");

                    b.Property<DateTime?>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("DateUpdated");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("DeletedAt");

                    b.Property<byte[]>("EncryptedDetails")
                        .IsRequired()
                        .HasMaxLength(262144)
                        .HasColumnType("bytea")
                        .HasColumnName("EncryptedDetails");

                    b.Property<byte[]>("EncryptedOverview")
                        .IsRequired()
                        .HasMaxLength(262144)
                        .HasColumnType("bytea")
                        .HasColumnName("EncryptedOverview");

                    b.Property<Guid>("IconId")
                        .HasColumnType("uuid")
                        .HasColumnName("IconId");

                    b.Property<bool>("IsArchive")
                        .HasColumnType("boolean")
                        .HasColumnName("IsArchive");

                    b.Property<bool>("IsFavorite")
                        .HasColumnType("boolean")
                        .HasColumnName("IsFavorite");

                    b.Property<bool>("IsInTrash")
                        .HasColumnType("boolean")
                        .HasColumnName("IsInTrash");

                    b.Property<string>("PasswordType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("PasswordType");

                    b.Property<Guid[]>("Tags")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("TagsIds");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("UserId");

                    b.HasKey("Id");

                    b.ToTable("VaultItems", (string)null);
                });

            modelBuilder.Entity("EnigmaVault.PasswordService.Domain.Models.Folder", b =>
                {
                    b.HasOne("EnigmaVault.PasswordService.Domain.Models.Folder", null)
                        .WithMany()
                        .HasForeignKey("ParentFolderId")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
