...
{
    public class UnArchiveVaultCommandHandlerTests : IClassFixture<TestFixture>
    {
        private readonly TestFixture _fixture;
        private readonly UnArchiveVaultCommandHandler _handler;
        private readonly CancellationToken _cancellationToken = default;

        public UnArchiveVaultCommandHandlerTests(TestFixture fixture)
        {
            _fixture = fixture;
            ...
            _handler = new UnArchiveVaultCommandHandler(vaultRepo, unitOfWork);
        }
        
        [Fact]
        public async Task Handle_ValidCommand_UnArchive()
        {
            ...
            var command = new UnArchiveVaultCommand(vault.Id, userId);
            ...
            vaultInDb?.IsArchive.Should().BeFalse();
        }
    
        [Fact]
        public async Task Handle_ValidCommand_NotFoundVaultItemReturnNotFoundError()
        {
            var command = new UnArchiveVaultCommand(VaultItemId.Create(Guid.NewGuid()), UserId.Create(Guid.NewGuid()));
            Result result = await _handler.Handle(command, _cancellationToken);
