...
{
    public class ArchiveVaultCommandHandlerTests : IClassFixture<TestFixture>
    {
        private readonly TestFixture _fixture;
        private readonly ArchiveVaultCommandHandler _handler;
        private readonly CancellationToken _cancellationToken = default;

        public ArchiveVaultCommandHandlerTests(TestFixture fixture)
        {
            _fixture = fixture;
            ...
            _handler = new ArchiveVaultCommandHandler(vaultRepo, unitOfWork);
        }
        
        [Fact]
        public async Task Handle_ValidCommand_AddToArchive()
        {
            ...
            var command = new ArchiveVaultCommand(vault.Id, userId);
            ...
            vaultInDb?.IsArchive.Should().BeTrue();
        }
    
        [Fact]
        public async Task Handle_ValidCommand_NotFoundVaultItemReturnNotFoundError()
        {
            var command = new ArchiveVaultCommand(VaultItemId.Create(Guid.NewGuid()), UserId.Create(Guid.NewGuid()));
            Result<Unit> result = await _handler.Handle(command, _cancellationToken);
